...
import { deleteMutation, switchMutation, useTodosQuery } from '../../../query/useTodosQuery';
...
function Todo({ todo, isActive }) {
  ...
  const { deleteMutation, switchMutation } = useTodosQuery();

  const { mutate: deleteMutate } = deleteMutation;
  const { mutate: switchMutate } = switchMutation;
